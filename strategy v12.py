...
def compute_m15_break_confirm(m15_df, h1_df):
    ...
    for _, m15_row in m15_df.iterrows():
        ...
        if h1_ptr < 0:
            ...
            line_val_up_list.append(np.nan)
            line_val_down_list.append(np.nan)
            continue
...
def compute_entry_triggers(m15_break_df, m5_df):
    ...
    for _, row in m15_break_df.iterrows():
        trigger_time = row["timestamp"] + pd.Timedelta(minutes=25)
...
LTF_CANDIDATES = ["5m"]  # تایم مانیتورینگ/ورود به ۵ دقیقه تغییر کرد - برای تشخیص دقیق لحظه‌ی ورود (۲۵ دقیقه بعد از شکست) لازمه
...
def run_one_timeframe(exchange, symbol, ltf, m15_break_df, atr_tf_df):
    ...
    print("در حال محاسبه‌ی لحظه‌ی دقیق ورود (۲۵ دقیقه بعد از شکست)...")
    ...
    ltf_df = merge_line_to_ltf(ltf_df, m15_break_df)
