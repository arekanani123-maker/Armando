...
TP_ATR_MULT = 3.0         # TP = ورود ± ۳×ATR (استاندارد حرفه‌ای رایج‌تر - مثل chandelier exit)
...
def backtest(ltf_df, triggers, reversal_triggers, allowed_side):
    ...
    for i in range(max(PIVOT_LEN * 2, 20), n):
        ...
        if position is not None:
            ...
            # تارگت دوم: شکست معکوس همون خط‌روند - مانیتورینگش به یک‌ساعته تغییر
            # کرد، فقط لحظه‌ی بسته‌شدن هر کندل H1 چک می‌شه (نه هر کندل ۱۵ دقیقه).
...
LTF_CANDIDATES = ["5m"]  # تایم مانیتورینگ/ورود به ۵ دقیقه تغییر کرد - برای دقت واقعی‌تر تاخیر ۱۹ دقیقه‌ای
...
def run_one_timeframe(exchange, symbol, ltf, h1_break_df):
    ...
    print("در حال محاسبه‌ی لحظه‌ی دقیق ورود (پول‌بک ۱۵ دقیقه‌ای، لحظه‌ی شکست خط H1)...")
