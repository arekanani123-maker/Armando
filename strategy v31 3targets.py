...
TP1_ATR_MULT = 3.04       # تارگت ۱ = ورود ± ۳.۰۴×ATR - ۴۰٪ پوزیشن
TP2_ATR_MULT = 4.04       # تارگت ۲ = ورود ± ۴.۰۴×ATR (دقیقاً وسط ۱ و ۳) - ۳۰٪ پوزیشن
TP3_ATR_MULT = 5.04       # تارگت ۳ = ورود ± ۵.۰۴×ATR - ۳۰٪ پوزیشن
...
def backtest(ltf_df, triggers, reversal_triggers, allowed_side):
    ...
    position = None  # dict: side, entry, sl, tp1, tp2, tp3, qty1, qty2, qty3, part1_open/2/3_open, mfe
    trades = []
    ...
    for i in range(max(PIVOT_LEN * 2, 20), n):
        ...
        if position is not None:
            ...
            def close_remaining(exit_price, exit_reason, exit_ts):
                """هرچی از پوزیشن باز مونده (هر کدوم از سه بخش) رو با یه قیمت می‌بنده."""
                nonlocal equity
                qty_remaining = 0.0
                if position["part1_open"]:
                    qty_remaining += position["qty1"]
                if position["part2_open"]:
                    qty_remaining += position["qty2"]
                if position["part3_open"]:
                    qty_remaining += position["qty3"]
                if qty_remaining <= 0:
                    return
                if side == "long":
                ...
                position["part1_open"] = False
                position["part2_open"] = False
                position["part3_open"] = False

            def close_part(part_num, tp_price, reason):
                nonlocal equity
                qty = position[f"qty{part_num}"]
                if side == "long":
                    pnl = qty * (tp_price - position["entry"])
                    mfe_pct = (position["mfe"] - position["entry"]) / position["entry"] * 100
                else:
                    pnl = qty * (position["entry"] - tp_price)
                    mfe_pct = (position["entry"] - position["mfe"]) / position["entry"] * 100
                fee = (qty * position["entry"] + qty * tp_price) * (COMMISSION_PCT / 100)
                pnl -= fee
                equity += pnl
                trades.append({
                    "side": side, "entry_time": position["entry_time"], "entry": position["entry"],
                    "exit_time": ts, "exit": tp_price, "reason": reason,
                    "pnl": pnl, "equity_after": equity, "mfe_pct": mfe_pct,
                    "sl": position["sl"], "tp": tp_price,
                })
                position[f"part{part_num}_open"] = False

            # ---- SL - هرچی از پوزیشن باز مونده رو می‌بنده ----
            # (بعد از خوردن تارگت۲، این سطح به نقطه‌ی ورود منتقل می‌شه - بریک‌ایون)
            ...
            # ---- تارگت ۱ (۴۰٪ پوزیشن) ----
            if position is not None and position["part1_open"]:
                hit = (side == "long" and high >= position["tp1"]) or (side == "short" and low <= position["tp1"])
                if hit:
                    close_part(1, position["tp1"], "take_profit_tp1")

            # ---- تارگت ۲ (۳۰٪ پوزیشن) - وقتی بخوره، SL باقی‌مونده بریک‌ایون می‌شه ----
            if position is not None and position["part2_open"]:
                hit = (side == "long" and high >= position["tp2"]) or (side == "short" and low <= position["tp2"])
                if hit:
                    close_part(2, position["tp2"], "take_profit_tp2")
                    position["sl"] = position["entry"]  # بریک‌ایون برای باقی‌مونده (تارگت۳)

            # ---- تارگت ۳ (۳۰٪ پوزیشن) ----
            if position is not None and position["part3_open"]:
                hit = (side == "long" and high >= position["tp3"]) or (side == "short" and low <= position["tp3"])
                if hit:
                    close_part(3, position["tp3"], "take_profit_tp3")

            if position is not None and not position["part1_open"] and not position["part2_open"] and not position["part3_open"]:
                position = None
        ...
        if position is None and ts in triggers:
            ...
            # سایز پوزیشن: مارجین ثابت × لوریج / قیمت ورود، تقسیم ۴۰٪/۳۰٪/۳۰٪
            qty_total = (POSITION_MARGIN * LEVERAGE) / entry_price
            qty1 = qty_total * TP1_FRACTION
            qty2 = qty_total * TP2_FRACTION
            qty3 = qty_total * TP3_FRACTION
            ...
            if trig["side"] == "long" and allowed_side in ("long", "both") and not_sideways and not pd.isna(breakout_price):
                ...
                if entry_price > sl_price:
                    position = {
                        ...
                        "tp1": entry_price + TP1_ATR_MULT * atr_val,
                        "tp2": entry_price + TP2_ATR_MULT * atr_val,
                        "tp3": entry_price + TP3_ATR_MULT * atr_val,
                        "qty1": qty1, "qty2": qty2, "qty3": qty3,
                        "part1_open": True, "part2_open": True, "part3_open": True,
