...
def backtest(ltf_df):
    ...
    for i in range(max(PIVOT_LEN * 2, 20), n):
        ...
        # ---- بررسی سیگنال ورود جدید - فقط فیلتر شکست خط‌روند ----
...
LTF_CANDIDATES = ["3m", "5m"]  # مقایسه‌ی دو تایم ورود، طبق درخواست


def run_one_timeframe(exchange, ltf, m15_break_df):
    print(f"\n--- تایم‌فریم ورود: {ltf} ---")
...
def main():
    ...
    for ltf in LTF_CANDIDATES:
        res = run_one_timeframe(exchange, ltf, m15_break_df)
        results.append(res)
