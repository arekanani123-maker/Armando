# -*- coding: utf-8 -*-
"""
استراتژی ۷ - مستقل و کاملاً جدا از استراتژی ۶
------------------------------------------------------------------
شکست خط‌روند، با ورود دقیق روی پول‌بک، تارگت‌گیری دو-نیمه، و تارگت۲ به‌شکل
شکست معکوس خط‌روند (چک شده در هر دوره‌ی TREND_TF).

فرق‌های کلیدی با استراتژی ۶:
- خط‌روند از سوینگ‌های ۳۰ دقیقه‌ای (نه ۱ ساعته)
- ورود با پول‌بک ۵ دقیقه‌ای (نه ۱۵ دقیقه‌ای)
- تارگت۲ هر ۳۰ دقیقه چک می‌شه (نه هر ساعت)
- ADX: دوره ۷، آستانه‌ی ۱۶ (نه دوره ۱۴، آستانه‌ی ۲۱)

بقیه (ATR از ۱۵ دقیقه، SL=۳٪، TP اصلی=۳.۰۴×ATR، تارگت۳=۵.۰۴×ATR، تقسیم
۵۰٪/۵۰٪ پوزیشن، ۶ نماد) عیناً مثل استراتژی ۶.

نحوه اجرا (روی Replit/Colab):
    pip install ccxt pandas numpy
    python strategy_7.py
"""
...
# ══════════════════════════════════════════════════════════════════
# 🔒 تنظیمات قفل‌شده (استراتژی «۷» - کاملاً مستقل از استراتژی ۶)
# ══════════════════════════════════════════════════════════════════
SYMBOLS = ["XRP/USDT:USDT", "ETH/USDT:USDT"]  # فقط دو نماد - تغییر کرد
TREND_TF = "30m"          # تایم‌فریم رسم خط‌روند (از سوینگ‌های ۳۰ دقیقه)
TREND_TF_MINUTES = 30     # طول همون تایم‌فریم به دقیقه - برای محاسبه‌ی پنجره‌ی ورود/تارگت۲
ATR_TF = "15m"            # ATR برای TP - از ۱۵ دقیقه‌ای محاسبه می‌شه
MONTHS_BACK = 6           # طول خودِ بازه‌ی تست - برگشت به ۶ ماه
...
SYMBOL_DIRECTION = {
    "XRP/USDT:USDT": "both",   # هردو جهت آزاد
    "ETH/USDT:USDT": "both",   # هردو جهت آزاد
}
...
ADX_LEN = 14               # دوره‌ی ADX - تغییر کرد
ADX_MIN = 21               # آستانه‌ی ADX - تغییر کرد
ADX_ENABLED = True

PIVOT_LEN = 5             # Pivot Left/Right bars (برای رسم خط‌روند - تایم TREND_TF)
...
def add_htf_indicators(df):
    """فقط pivot_high/pivot_low لازمه - برای رسم خط‌روند."""
...
def compute_h1_break(h1_df):
    """
    شکست بر پایه‌ی بالاترین/پایین‌ترین قیمت کندل TREND_TF نسبت به خط چک می‌شه (نه
    ...
    است - نه قیمت بسته یا هر قیمت دیگه‌ی کندل. برای هر کندل TREND_TF، سوینگ‌پوینت‌ها
...
def compute_reversal_triggers(h1_break_df):
    """
    تارگت دوم: مانیتورینگش به‌جای هر کندل ۱۵ دقیقه، فقط یک‌بار در هر تایم‌فریم
    خط‌روند (TREND_TF_MINUTES دقیقه - لحظه‌ی بسته‌شدن هر کندل TREND_TF) چک
    می‌شه که آیا قیمت دوباره همون خط‌روند رو در جهت مخالف شکسته یا نه.
    خروجی: دیکشنری از check_time -> {line_val_up, line_val_down}.
    """
    triggers = {}
    for _, row in h1_break_df.iterrows():
        check_time = row["timestamp"] + pd.Timedelta(minutes=TREND_TF_MINUTES)
        triggers[check_time] = {
...
def compute_entry_triggers(h1_break_df, m15_df):
    """
    ورود دیگه منتظر بسته‌شدن کندل TREND_TF نمی‌مونه. برای هر پنجره‌ی
    TREND_TF_MINUTES دقیقه‌ای، خط‌روند از قبل (بر اساس سوینگ‌های TREND_TF
    *قبل از* همین پنجره) مشخصه. کندل‌های ورود (LTF) داخل همون پنجره
    (پول‌بک‌ها) یکی‌یکی به ترتیب زمان چک می‌شن - اولین کندلی که high/low ش
    از خط رد بشه، همون لحظه‌ی ورود می‌شه. قیمت ورود دقیقاً قیمت خودِ
    خط‌روند (نه قیمت کندل) - بدون نگاه به آینده.
    ...
    for _, h1_row in h1_break_df.iterrows():
        ...
        period_end = period_start + pd.Timedelta(minutes=TREND_TF_MINUTES)
...
def backtest(ltf_df, triggers, reversal_triggers, allowed_side):
    ...
    for i in range(max(PIVOT_LEN * 2, 20), n):
        ...
        # ---- بررسی سیگنال ورود جدید - دقیقاً روی کندل تریگر (لحظه‌ی شکست خط‌روند) ----
...
LTF_CANDIDATES = ["5m"]  # تایم مانیتورینگ/ورود - پول‌بک‌های ۵ دقیقه‌ای (تغییر کرد)


def print_full_trades_table(trades_df, label):
    """
    خلاصه‌ی کوتاه به‌جای چاپ همه‌ی معاملات (که با هزاران معامله خیلی طولانی
    می‌شد) - جزئیات کامل همیشه توی فایل CSV ذخیره می‌شه، همون‌جا چکش کن.
    """
    print(f"\n{label}: {len(trades_df)} معامله ثبت شد (جزئیات کامل توی فایل CSV)")
...
def run_one_timeframe(exchange, symbol, ltf, h1_break_df, atr_df):
    ...
    print(f"در حال محاسبه‌ی لحظه‌ی دقیق ورود (پول‌بک {ltf}، لحظه‌ی شکست خط‌روند {TREND_TF})...")
