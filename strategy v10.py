...
def backtest(ltf_df):
    ...
    for i in range(max(PIVOT_LEN * 2, 20), n):
        ...
        # ---- بررسی سیگنال ورود جدید - شکست خط‌روند + ADX، بدون سقف روزانه ----
        if position is None:
            not_sideways = row["adx"] >= ADX_MIN
            buy_cond = ALLOW_LONG and row.get("break_up", False) and not_sideways
            sell_cond = ALLOW_SHORT and row.get("break_down", False) and not_sideways
            ...
            qty = (POSITION_MARGIN * LEVERAGE) / close
...
LTF_CANDIDATES = ["1h"]  # تایم ورود تغییر کرد به ۱ ساعته
