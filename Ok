    ...
    for ts in ltf_df["timestamp"]:
        ...
        while htf_ptr + 1 < len(htf_times) and htf_times[htf_ptr + 1] < np.datetime64(ts):
            ...
            if rec["pivot_high"]:
                ...
                sw_high1, sw_high1_idx = rec["high"], htf_ptr
            if rec["pivot_low"]:
                sw_low2, sw_low2_idx = sw_low1, sw_low1_idx
                sw_low1, sw_low1_idx = rec["low"], htf_ptr
        ...
        # فیلتر مستقل شکست خط‌روند (مستقل از پروژه‌ی انجل، فقط همون تکنیک عمومی):
        # خط نزولی از دو سقف سوینگ آخر وصل میشه - قیمت باید بالاش بسته بشه (لانگ).
        # خط صعودی از دو کف سوینگ آخر وصل میشه - قیمت باید پایینش بسته بشه (شورت).
        # اگر هنوز ۲ سوینگ‌پوینت معتبر نداریم، سخت‌گیر نیستیم - True برمی‌گردونیم
        # (مثل رفتار مشابه در فیلتر ترندلاین انجل، ولی اینجا از صفر نوشته شده).
        if sw_high1_idx is not None and sw_high2_idx is not None:
            trendline_break_up = htf_close_val > _line_value_at(
                sw_high2_idx, sw_high2, sw_high1_idx, sw_high1, htf_ptr)
        else:
            trendline_break_up = True
        if sw_low1_idx is not None and sw_low2_idx is not None:
            trendline_break_down = htf_close_val < _line_value_at(
                sw_low2_idx, sw_low2, sw_low1_idx, sw_low1, htf_ptr)
        else:
            trendline_break_down = True
        ...
        # (RSI/ADX/pullback/body ratio) میونه موندن. حالا شکست خط‌روند M15 هم به‌عنوان
        # شرط اضافه‌ی مستقل لازمه.
        bull_bias = (price_above or bull_structure) and slope_up and trendline_break_up
        bear_bias = (price_below or bear_structure) and slope_down and trendline_break_down
