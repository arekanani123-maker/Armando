...
RSI_LEN = 14
RSI_BUY_MIN = 55           # سخت‌گیرانه شد (قبلاً 45) - مومنتوم صعودی واقعی بخواد، نه فقط «خارج از فروش‌افراطی»
RSI_SELL_MAX = 45          # سخت‌گیرانه شد (قبلاً 55) - قرینه‌ی همون منطق

PIVOT_LEN = 5             # Pivot Left/Right bars (هم برای ساختار M15 هم SL سوینگ M5)

ADX_LEN = 14
ADX_MIN = 25               # سخت‌گیرانه شد (قبلاً 18) - فقط روند واقعاً قوی، نه هر روند ضعیف
MAX_ATR_DIST = 2.5        # حداکثر فاصله از EMA50 (ضریب ATR) - فیلتر "خیلی دور نشده"
PULLBACK_ATR_MAX = 0.6    # سخت‌گیرانه شد (قبلاً 1.0) - فقط پول‌بک تمیز و نزدیک به EMA

MIN_BODY_RATIO = 0.6      # سخت‌گیرانه شد (قبلاً 0.4) - کندل تاییدی باید قاطع باشد
...
def merge_htf_bias(ltf_df, htf_df):
    ...
    for ts in ltf_df["timestamp"]:
        ...
        # سخت‌گیرانه شد: قبلاً کافی بود فقط قیمت بالای EMA *یا* ساختار صعودی باشد؛
        # الان هر دو هم‌زمان لازمند، به‌علاوه شیب EMA - یعنی روند M15 باید واقعاً
        # هم از نظر قیمت هم از نظر ساختار سوینگ (HH/HL) تایید بشه، نه فقط یکی.
        bull_bias = price_above and bull_structure and slope_up
        bear_bias = price_below and bear_structure and slope_down
