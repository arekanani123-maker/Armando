...
def compute_entry_triggers(m15_break_df, m5_df):
    """
    تغییر: ورود دیگه بلافاصله بعد از تایید شکست (کندل ۱۵ دقیقه) انجام نمی‌شه.
    کندل ۱۵ دقیقه‌ای که شکست توش تایید شده، ورود توش ممنوعه. بعدش، با تایم‌فریم
    ۵ دقیقه، کندل اول (اولین ۵ دقیقه بعد از بسته‌شدن کندل ۱۵دقیقه‌ای) هم ممنوعه؛
    ورود دقیقاً روی کندل دوم ۵ دقیقه‌ای انجام می‌شه - یعنی دقیقاً ۲۰ دقیقه بعد از
    شروع کندل ۱۵دقیقه‌ای که شکست توش تایید شده، هم‌جهت با همون شکست.
    قیمت ورود = قیمت باز (open) همون کندل ۵ دقیقه‌ای (لحظه‌ی دقیقه‌ی بیستم).
    SL همچنان بر پایه‌ی قیمت بسته‌ی همون کندل ۱۵ دقیقه‌ای محاسبه می‌شه (بدون تغییر).
    خروجی: دیکشنری از trigger_time -> {side, entry_price, breakout_price}
    """
    m5_lookup = m5_df.set_index("timestamp")["open"]
    triggers = {}
    for _, row in m15_break_df.iterrows():
        trigger_time = row["timestamp"] + pd.Timedelta(minutes=20)
        if row["break_up"] and trigger_time in m5_lookup.index:
            triggers[trigger_time] = {
                "side": "long",
                "entry_price": m5_lookup.loc[trigger_time],
                "breakout_price": row["breakout_price_up"],
            }
        if row["break_down"] and trigger_time in m5_lookup.index:
            triggers[trigger_time] = {
                "side": "short",
                "entry_price": m5_lookup.loc[trigger_time],
...
def backtest(ltf_df, triggers):
    ...
    for i in range(max(PIVOT_LEN * 2, 20), n):
        ...
        # ---- بررسی سیگنال ورود جدید - دقیقاً روی کندل تریگر (۲۰ دقیقه بعد از شکست) ----
        if position is None and ts in triggers:
            ...
            if trig["side"] == "long" and ALLOW_LONG and not_sideways and not pd.isna(breakout_price):
            ...
            elif trig["side"] == "short" and ALLOW_SHORT and not_sideways and not pd.isna(breakout_price):
                sl_price = breakout_price * (1 + SL_BREAKOUT_PCT / 100)
...
LTF_CANDIDATES = ["5m"]  # تایم مانیتورینگ/ورود به ۵ دقیقه تغییر کرد - برای تشخیص دقیق لحظه‌ی ورود (۲۰ دقیقه بعد از شکست) لازمه
...
def run_one_timeframe(exchange, symbol, ltf, m15_break_df, atr_tf_df):
    ...
    print("در حال محاسبه‌ی لحظه‌ی دقیق ورود (۲۰ دقیقه بعد از شکست، کندل دوم ۵ دقیقه‌ای)...")
    triggers = compute_entry_triggers(m15_break_df, ltf_df)
    ...
    final_equity, trades_df = backtest(ltf_df, triggers)
...
def run_one_symbol(exchange, symbol):
    ...
    for ltf in LTF_CANDIDATES:
        res = run_one_timeframe(exchange, symbol, ltf, m15_break_df, atr_tf_df)
        results.append(res)
    return results
